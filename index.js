...
// ==========================================
const TOKEN_RAW_URL =
  "https://raw.githubusercontent.com/USERNAME/REPO/main/tokens.json";

async function getRawTokens() {
  try {
    const response = await axios.get(TOKEN_RAW_URL, {
      timeout: 15000,
      headers: {
    ...
    // Support format: ["TOKEN1", "TOKEN2"]
    if (Array.isArray(data)) {
      return data;
    }

    // Support format: { "tokens": ["TOKEN1", "TOKEN2"] }
    if (Array.isArray(data?.tokens)) {
      return data.tokens;
    }

    throw new Error("Format tokens.json tidak valid");
  } catch (err) {
    console.error(
      chalk.red("❌ Gagal mengambil token dari GitHub Raw:"),
      err.message
    );
...
async function validateToken() {
  ...
  try {
    ...
    // Token resmi divalidasi langsung dari GitHub Raw.
    const tokens = await getRawTokens();
    const isValid = tokens.some((item) => {
      if (typeof item === "string") return item.trim() === BOT_TOKEN.trim();
      if (item && typeof item === "object") {
        return item.tokenBot === BOT_TOKEN && item.isActive !== false;
      }
      return false;
    });

    if (!isValid) {
      console.log(chalk.red("\n╞═════⪨ Token tidak terdaftar di GitHub Raw ⪩═════╡\n"));
      return false;
    }

    console.log(chalk.green("✅ Token terverifikasi dari GitHub Raw"));
    return true;
  } catch (err) {
    console.log(chalk.red("⌭ Failed mengambil data Token dari GitHub Raw:"), err.message);
    return false;
  }
}
...
// ==========================================
// SISTEM AUTO UPDATE KODE VIA TELEGRAM
// ==========================================
...
const UPDATE_TEMP_PATH = "./index.js.update.tmp.js";
const UPDATE_BACKUP_PATH = "./index.js.backup";
...
function checkJsSyntax(filePath) {
  try {
    // Node.js --check membutuhkan ekstensi .js.
    ...
    return {
      ok: false,
      error: String(err.stderr || err.stdout || err.message || err).trim(),
    };
  }
}
...
┃ ⨳ Status : 🔎 Memeriksa update index.js
┃ ⨳ Source : GitHub Repository
┃ ⨳ Process : Downloading...
╰━━━━━━━━━━━━━━━━⬣

⏳ <b>Sedang melakukan sinkronisasi script...</b></pre>`,
    { parse_mode: "HTML" }
  );

  try {
    // Selalu download ke file .js sementara agar node --check tidak
    // gagal hanya karena ekstensi .tmp.
    try { fs.unlinkSync(UPDATE_TEMP_PATH); } catch (_) {}

    await downloadToFile(UPDATE_URL, UPDATE_TEMP_PATH);

    const stat = fs.statSync(UPDATE_TEMP_PATH);
    if (!stat.size) {
      throw new Error("File update kosong.");
    }

    const syntax = checkJsSyntax(UPDATE_TEMP_PATH);
    if (!syntax.ok) {
      throw new Error(`Syntax update tidak valid:\n${syntax.error}`);
    }

    // Backup index.js lama sebelum diganti.
    try {
      fs.copyFileSync(UPDATE_FILE_PATH, UPDATE_BACKUP_PATH);
    } catch (_) {}

    // Setelah lolos syntax, baru pasang update.
...
♻️ <b>Bot akan restart menggunakan index.js baru.</b>
🔐 <i>Validasi token tetap dilakukan oleh index.js sendiri.</i></pre>`,
      { parse_mode: "HTML" }
    );
    ...
    await ctx.reply(
`<pre>╭━━〔 ❌ 𝗨𝗣𝗗𝗔𝗧𝗘 𝗙𝗔𝗜𝗟𝗘𝗗 ❌ 〕━━⬣
┃ ⨳ Status : ❌ Update dibatalkan
┃ ⨳ File   : index.js lama tetap aman
┃ ⨳ Action : Tidak restart
╰━━━━━━━━━━━━━━━━⬣</pre>

<b>Update gagal melewati pemeriksaan.</b>

<blockquote><code>${String(e.message || e).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</code></blockquote>`,
      { parse_mode: "HTML" }
    );
  }
}
